...
- 인텔 `듀얼 코어 CPU` 에 Hyper Threading이 적용됐다면, 하드웨어 스레드는 총 몇 개일까?
    ...
    - 하드웨어 스레드는 총 4개이다.
    - OS는 해당 CPU를 `쿼드 코어(4개 코어)` 로 인식한다.
...
- `유저 스레드` 와 `OS 스레드` 가 `1:1` 로 연결되는 모델이다.
...
- `유저 스레드` 는 여러개가 있고, `OS 스레드` 는 딱 1개만 있는 모델이다.
    - `유저 스레드` 와 `OS 스레드` 가 `N:1` 로 연결되는 모델
- 컨텍스트 스위칭이 더 빠르다.
- 컨텍스트 스위칭이 더 빠른이유는 `유저 레벨` 에서만 일어나고, `커널` 이 전혀 개입하지 않는다. `커널` 이 전혀 개입하지 않기 때문에 컨텍스트 스위칭이 더 빠르다.
- `유저 스레드` 간 컨텍스트 스위칭이 `One-to-One` 모델보다 빠르다.
- `OS 스레드` 는 1개이고, 싱글 스레드이기 때문에 `커널(OS 레벨)` 에서 `race condition` 이 발생할 가능성은 거의 없다.
- `race condition` 이 일어난다면, `유저 레벨` 에서 일어난다.
    - `유저 스레드` 가 컨텍스트 스위칭을 하기 때문에 `race condition` 이 일어난다.
- `OS 스레드` 가 1개이기 때문에 멀티코어를 활용 못한다.
- 하나의 `유저 스레드` 가 `Block` 상태가 되면, `OS 스레드` 도 `Block` 상태가 되기 때문에 다른 `유저 스레드` 들도 전부 `Block` 이 된다.
    - 이 문제를 해결하기 위에 `Non-Blocking I/O` 를 사용한다.

#### Many-to-Many Model

- `One-to-One` 모델과 `Many-to-One` 모델의 장점을 합쳐서 만든것이다.
- `Go` 언어가 해당 모델을 지원한다고 한다.

### 3개의 스레딩 모델 도식화

<img width="777" alt="스크린샷 2023-06-14 오후 11 28 04" src="https://github.com/bestdevhyo1225/dev-log/assets/23515771/d46d9e3e-570b-4805-a260-f28a80c0dbf8">

### 조금 다른 의미의 유저 스레드

- OS와는 독립적으로 `유저 레벨` 에서 스케줄링 되는 스레드를 `유저 스레드` 라고 한다.
- `One-to-Many` 나 `Many-to-Many` 모델에서 `유저 스레드` 들을 한정해서 지칭하기도 한다.

### 그린(Green) 스레드

- Java 초창기 버전에서는 `Many-to-One` 스레딩 모델을 사용했는데, 여기서 `유저 스레드` 들을 `그린 스레드` 라고 호칭했다.
- 오늘날 개념이 계속 확장이 됐고, OS와는 독립적으로 `유저 레벨` 에서 스케줄링 되는 스레드를 `그린 스레드` 라고 부르게 됐다.
- `Many-to-One`, `Many-to-Many` 에서의 `유저 스레드` 들을 `그린 스레드` 로 부르기도 한다.

### 유저 스레드와 그린(Green) 스레드

- 맥락에 따라서 `유저 스레드` 와 `그린 스레드` 는 같은 의미로 사용하기도 한다.

## 조금 다른 맥락에서 커널 스레드

- OS 커널의 역할을 수행하는 스레드
- `커널 스레드` 가 `OS 레벨` 에서 정의된 일반적으로 우리가 지금까지 알고 있었던 스레드의 의미로 `커널 스레드` 로 사용하기도 한다.
- `커널` 이 하는일을 수행하는 `커널 스레드` 로 의미가 사용될 때도 있다.

## 유저 레벨에서 스케줄링 되는 스레드

- 유저 레벨에서 스케줄링 되는 스레드는 `코루틴(Coroutine)` 과 관련이 있다.
