...
프로젝트를 진행하던 중... 문제가 없을거라고 생각하고 그냥 진행했던 `@OneToOne` 양방향 매핑에 대해서 정리해보고자 한다.
...
- 자식이 외래 키를 가지는 방식으로 설계되어 있다면? 위에서 테스트한 부모쪽 `@OneToOne`에는 `optional = false` 처리를 반드시 해주고,자식쪽 `@OneToOne`에는 `@MapsId`를 추가해서 문제를 해결해야 한다.
