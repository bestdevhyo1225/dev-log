...
- 테이블 간의 조인시, `WHERE` 조건이 있는 경우에는 `WHERE` 조건에 만족하는 `레코드가 적은 테이블` 을 `드라이빙 테이블` 로 선택한다.
    - `WHERE` 조건이 없다면, `전체 레코드 수` 를 기준으로 `레코드가 적은 테이블` 을 `드라이빙 테이블` 로 선택한다.
    - 현재 `드라이빙 테이블` 의 레코드 수가 증가하여, MySQL 옵티마이저에 의해 `드리븐 테이블` 로 변경될 가능성은 없는지?
    - 즉, 각각의 테이블에 데이터가 많이 쌓여도 `드라이빙 테이블`, `드리븐 테이블` 이 변경되지 않도록 주의해야한다.
    - 만약에 `드라이빙 테이블` 과 `드리븐 테이블` 이 변경되는 상황이 발생하면, `STRAIGHT_JOIN` 을 사용해서 `드라이빙 테이블` 과 `드리븐 테이블` 의 순서가 변경되지 않도록 하자

## Application 레벨에서 Join을 지양해야 하는 이유
